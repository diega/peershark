...
pub async fn handle_client(
    ...
    let eth_version = filtered_capabilities
        .iter()
        .filter(|c| c.name == "eth")
        .map(|c| c.version)
        .max()
    ...
    for attempt in 0..MAX_PEER_RETRIES {
        ...
        let peer_address = extract_peer_address(&peer_url).unwrap_or_default();
        let peer_node_id = extract_peer_node_id(&peer_url).unwrap_or_default();

        event_bus.emit(ProxyEvent::PeerConnecting {
            tunnel_id: tunnel_id.clone(),
            client_node_id: client_node_id.clone(),
        ...
        event_bus.emit(ProxyEvent::PeerConnected {
            tunnel_id: tunnel_id.clone(),
            client_node_id: client_node_id.clone(),
            remote_node_id: peer_node_id.clone(),
            remote_address: peer_address.clone(),
            remote_client_id: peer_client_id.clone(),
    ...
    event_bus.emit(ProxyEvent::PeerDisconnected {
        ...
        reason: format!("all {} peer connection attempts failed", attempts),
        timestamp: now_millis(),
    });
...
/// Extract "host:port" from an enode URL.
fn extract_peer_address(enode_url: &str) -> Option<String> {
    let parsed = url::Url::parse(enode_url).ok()?;
    let host = parsed.host_str()?;
    let port = parsed.port()?;
    Some(format!("{}:{}", host, port))
}

/// Extract the node_id (64 bytes hex) from an enode URL.
fn extract_peer_node_id(enode_url: &str) -> Option<String> {
    let parsed = url::Url::parse(enode_url).ok()?;
