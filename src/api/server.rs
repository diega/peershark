            ...
            match event_rx.recv().await {
                Ok(event) => match event {
                    ProxyEvent::ClientConnected {
                        ...
                        state
                            .add_client_connection(&client_node_id, &client_id, &tunnel_id)
                            .await;
                        debug!(tunnel_id, "client connection added to state");
                    }
                    ProxyEvent::PeerConnecting {
                        tunnel_id,
                        client_node_id,
                        peer_node_id,
                        peer_address,
                        ...
                        state
                            ...
                            .await;
                        debug!(tunnel_id, attempt_number, "peer attempt added to state");
                    }
                    ProxyEvent::PeerRejected {
                        tunnel_id,
                        client_node_id,
                        reason,
                        failure_kind,
                        ...
                        state
                            ...
                            .await;
                        debug!(tunnel_id, "peer attempt marked rejected");
                    }
                    ProxyEvent::PeerConnected {
                        ...
                        state
                            ...
                            .await;
                        debug!(tunnel_id, "connection activated in state");
                    }
                    ProxyEvent::PeerDisconnected {
                        tunnel_id,
                        client_node_id,
                        ..
                    } => {
                        state.remove_connection(&client_node_id, &tunnel_id).await;
                        debug!(tunnel_id, "connection removed from state");
                    }
                    ProxyEvent::MessageRelayed {
                        ...
                        state
                            .record_transfer(&client_node_id, &tunnel_id, bytes_in, bytes_out)
                            .await;
                        state.global_rate.record();
                    }
                    ProxyEvent::TrafficTick { .. } => {
                        // Traffic ticks don't update state
                    }
                },
